...
node* pke(node* head, int n, int position);
...
void bubbleSort(Node* head, int n) {
    ...
    Node* now = head;
        ...
        for (int j = 0; j < n - i - 1; j++) {
            ...
            printf("\nLinked list sorted using Bubble Sort.\n\n");}
            else{printf("\nLinked list empty nothing to Sort.\n\n");}
...
node* createNode(node* head)
{
    ...
    if(temp!=NULL)
    {
        while(temp != NULL)
        {
            if (nn->data!=temp->data)   temp=temp->next;
...
node* insert_head(node* head, node* nn)
{
    ...
    }else if (head!=NULL && nn == NULL){head=temp;}
    else if (head==NULL && nn != NULL){    
        head=nn;
    printf("Node inserted in Head!\n\n");
    return head;}
    return head;
...
node* insert_tail(node* head, node* nn)
{
    node* temp = head;if(nn==NULL)    return temp;
    if(temp==NULL)
    {
        head = insert_head(head,nn);
    }
